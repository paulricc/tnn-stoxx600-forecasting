...
def main() -> None:
    """Run the seed variance experiment for LSTM and TNN."""
    config = load_config()
    ...
    for seed in SEEDS:
        ...
        logger.info("seed %d — LSTM RMSE: %.5f", seed, lstm_rmse)
        ...
        logger.info("seed %d — TNN RMSE: %.5f", seed, tnn_rmse)

    print(f"\n=== Test RMSE over {len(SEEDS)} seeds (horizon={HORIZON}) ===")
    summary = []
